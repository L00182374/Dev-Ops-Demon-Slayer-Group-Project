...
from src.projectToTest import lessThan, greaterThan, equalTo, notEqualTo# import functions to be tested

#logger = logging.getLogger(__name__)# Create a logger for this test module

@pytest.mark.parametrize("a,b,expected", [# define test cases for lessThan
    (1, 2, True),
    (2, 1, False),
    (1, 1, False),
])
...
@pytest.mark.parametrize("a,b,expected", [# define test cases for greaterThan
    (2, 1, True),
    (1, 2, False),
    (1, 1, False),
])
...
@pytest.mark.parametrize("a,b,expected", [# define test cases for equalTo
    (2, 2, True),
    (1, 2, False),
    (2, 1, False),
])
...
@pytest.mark.parametrize("a,b,expected", [# define test cases for notEqualTo
    (1, 2, True),
    (2, 2, False),
    (2, 1, True),
])
...
@pytest.mark.parametrize("func", [lessThan, greaterThan, equalTo, notEqualTo])# define test cases for invalid inputs
def test_invalid_inputs_raise_typeerror(func):# define a test method for invalid inputs 
